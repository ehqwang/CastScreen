...
class WeakHandler {
    private var mCallback // hard reference to Callback. We need to keep callback in memory
     ...
     */
    fun WeakHandler(callback: Handler.Callback?) {
     ...
     * @param callback The callback interface in which to handle messages, or null.
     */
    fun WeakHandler(looper: Looper?, callback: Handler.Callback?) {
        mCallback = callback
    ...
    fun post(r: Runnable?): Boolean {
        return wrapRunnable(r)?.let { mExec?.post(it) } ?:false
    }
